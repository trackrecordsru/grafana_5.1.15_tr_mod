...
export const PageToolbar: FC<Props> = React.memo(
  ({
    ...
    const workspaceBlockMode = (window as any).inIframe() && workspaceBlockId;
    if (workspaceBlockMode) {
      return null;
    }

    return (
      <nav className={mainStyle} aria-label={ariaLabel}>
        {!(window as any).inIframe() && pageIcon && !onGoBack && (
        ...
        <nav aria-label="Search links" className={styles.navElement}>
          {parent && parentHref && (
            <>
              <Link
                ...
                style={(window as any).inIframe() ? { color: '#0074b3' } : {}}
               /* href={(window as any).inIframe() ? '/' : parentHref}*/
                href={'/'}
              >
          ...
          {title && titleHref && (
            <h1 className={styles.h1Styles}>
              <span className={cx(styles.titleText, styles.titleLink)}>{title}</span>
            </h1>
          )}
