...
import { locationService, config as grafanaRunTimeConfig } from '@grafana/runtime';
...
const homeUrl = hostname.includes('beta') ? 'https://beta.trackrecords.ru/' : 'https://lk.trackrecords.ru/';
...
export const NavBarUnconnected = React.memo(({ navBarTree }: Props) => {
  ...
  const isAdmin = grafanaRunTimeConfig.bootData.user.isGrafanaAdmin;
  ...
  if (kiosk !== KioskMode.Off || (window as any).inIframe()) {
    return null;
  }
  ...
  return (
    <nav className={cx(styles.sidemenu, 'sidemenu')} data-testid="sidemenu" aria-label="Main menu">
      ...
      <NavBarSection>
        ...
        {isAdmin && <NavBarItem className={styles.search} isActive={activeItem === searchItem} link={searchItem}>
          <Icon name="search" size="xl" />
        </NavBarItem>}
      </NavBarSection>

      {isAdmin && <NavBarSection>
      ...
      {showSwitcherModal && isAdmin && <OrgSwitcher onDismiss={toggleSwitcherModal} />}
