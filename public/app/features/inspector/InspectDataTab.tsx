...
export class InspectDataTab extends PureComponent<Props, State> {
  ...
  public exportCsv = (dataFrame: DataFrame, csvConfig: CSVConfig = {}) => {
    ...
    const fileName = `${displayTitle}-data${transformation}-${dateTimeFormat(new Date())}.xlsx`; // Измените расширение на .xlsx

    // Создайте двоичные данные Excel
    const excelData = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });

    // Создайте Blob из двоичных данных и сохраните его
    ...
    return (
      <div className={styles.wrap} aria-label={selectors.components.PanelInspector.Data.content}>
        <div className={styles.toolbar}>
          ...
          <Button
            variant="primary"
            onClick={() => this.exportCsv(dataFrames[dataFrameIndex], {useExcelHeader: false})} /*useExcelHeader hardcode false to remove errors by downloading*/
            className={css`
              margin-bottom: 10px;
            `}
          >
